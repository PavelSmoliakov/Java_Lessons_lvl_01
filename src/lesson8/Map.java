...
public class Map extends JPanel {
    ...
    void modeTwoPlayer(int y, int x) {
        char cymbal =  stepPlayer2 ? COMP_DOT : HUMAN_DOT;
        playerStep(y,x,cymbal);
        repaint();
        if (checkWin(cymbal)) {
            gameOverWindow.setMessage(stepPlayer2 ? "Победил игрок 2" : "Победил игрок 1",this);
            return;
        }
        if (isFuelFull()) {
    ...
    void modePlayAI(int y, int x) {
        playerStep(y,x,HUMAN_DOT);
        repaint();
        if (checkWin(HUMAN_DOT)) {
            gameOverWindow.setMessage("Вы победили",this);
            return;
        }
        if (isFuelFull()) {
            gameOverWindow.setMessage("Ничья",this);
        ...
        if (checkWin(COMP_DOT)) {
            gameOverWindow.setMessage("Победил компьютер",this);
            return;
        }
        if (isFuelFull()) {
            gameOverWindow.setMessage("Ничья",this);
